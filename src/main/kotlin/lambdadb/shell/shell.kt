...
fun renderTable(terminal: Terminal, operator: PhysicalOperator) {
    ...
    terminal.writer().println(horizontalLine)
    terminal.writer().println(AttributedString("$rowCount rows returned", AttributedStyle.DEFAULT.foreground(AttributedStyle.CYAN)).toAnsi(terminal))
}
